...
export async function listProducts({ search = "", category = "", lowOnly = false } = {}) {
  ...
  const rows = await denis.query(`SELECT * FROM products${where.length ? " WHERE " + where.join(" AND ") : ""} ORDER BY name ASC LIMIT 500`);
  return lowOnly ? rows.filter((p) => p.quantity <= p.min_quantity) : rows;
}

export async function getProduct(id) {
  const rows = await denis.query(`SELECT * FROM products WHERE id = ${Number(id)} LIMIT 1`);
  return rows[0] ?? null;
}

export async function findBySku(sku) {
  const rows = await denis.query(`SELECT * FROM products WHERE sku = ${q(sku)} LIMIT 1`);
  return rows[0] ?? null;
}
...
export async function createProduct({ sku, name, category, unit, quantity, minQuantity, price }, actor) {
  ...
  await denis.execute(
    `INSERT INTO products (id, sku, name, category, unit, quantity, min_quantity, price, updated_at) VALUES (${id}, ${q(sku)}, ${q(name)}, ${q(category || "General")}, ${q(unit || "pcs")}, ${qty}, ${Math.max(0, Number(minQuantity) || 0)}, ${Number(price) || 0}, ${q(new Date().toISOString())})`,
  );
...
export async function updateProduct(id, { name, category, unit, minQuantity, price }) {
  ...
  await denis.execute(
    `UPDATE products SET name = ${q(String(name).trim() || product.name)}, category = ${q(category || product.category)}, unit = ${q(unit || product.unit)}, min_quantity = ${Math.max(0, Number(minQuantity) || 0)}, price = ${Number(price) || 0}, updated_at = ${q(new Date().toISOString())} WHERE id = ${Number(id)}`,
  );
  return getProduct(id);
}
...
export async function deleteProduct(id) {
  const product = await getProduct(id);
  if (!product) throw new Error("Product not found");
  await denis.execute(`DELETE FROM movements WHERE product_id = ${Number(id)}`);
  await denis.execute(`DELETE FROM products WHERE id = ${Number(id)}`);
  return product;
}
...
export async function move(id, { kind, quantity, note }, actor) {
  ...
  await denis.execute(`UPDATE products SET quantity = ${next}, updated_at = ${q(new Date().toISOString())} WHERE id = ${Number(id)}`);
...
async function recordMovement({ productId, sku, kind, quantity, note }, actor) {
  ...
  await denis.execute(
    `INSERT INTO movements (id, product_id, sku, kind, quantity, note, actor, created_at) VALUES (${id}, ${Number(productId)}, ${q(sku)}, ${q(kind)}, ${Number(quantity)}, ${q(String(note || "").trim().slice(0, 200))}, ${q(actor)}, ${q(new Date().toISOString())})`,
  );
}

export async function listMovements({ productId = null, limit = 100 } = {}) {
  const where = productId ? ` WHERE product_id = ${Number(productId)}` : "";
  return denis.query(`SELECT * FROM movements${where} ORDER BY id DESC LIMIT ${Math.min(500, Math.max(1, Number(limit)))}`);
}
...
export async function overview() {
  const { data } = await denis.graph(`{
    ...
    stock: table("products") { id sku name quantity min_quantity unit price }
    recent: table("movements", order: "id desc", limit: 8) { id sku kind quantity note actor created_at }
  }`);
...
export async function categories() {
  const rows = await denis.query("SELECT category FROM products LIMIT 500");
